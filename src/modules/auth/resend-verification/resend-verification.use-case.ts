...
import { OtpResendCooldownException } from '../../../common/exceptions/auth.exceptions';
...
export class ResendVerificationUseCase {
  ...
  async execute(dto: ResendVerificationDto): Promise<{ message: string }> {
    ...
    const cooldownRemaining = await this.otpService.getResendCooldownRemaining(
      user.id,
      OtpType.EMAIL_VERIFICATION,
    );
    if (cooldownRemaining > 0) {
      throw new OtpResendCooldownException(cooldownRemaining);
    }
